...
const MessageStore = Reflux.createStore({
  ...
  _processQueue: function () {
    ...
    if (canProcessNext) {
      ...
      this.orbit
        .send(task.channel, task.message)
        .then(post => {
          this.sending = false
  ...
  loadMessages: function (channel, amount, force, refresh) {
    if (this.channels[channel].messages.length > 0) {
      if (refresh) {
        ...
        this.orbit
          .get(channel, null, first, len + messagesBatchSize)
          .then(messages => {
            setImmediate(() => {
