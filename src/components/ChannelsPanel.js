...
class ChannelsPanel extends React.Component {
  ...
  _renderChannel (e) {
    const name = e.name
    const unreadMessagesCount = this.state.appState.unreadMessages[name]
      ? this.state.appState.unreadMessages[name]
      : 0
    const hasUnreadMessages = unreadMessagesCount > 0
    const mentionsCount = this.state.appState.mentions[name]
      ? this.state.appState.mentions[name]
      : 0
    ...
    return (
      <div className="row link" key={Math.random().toString()}>
        <span
          ...
          onClick={this.handleJoinChannel.bind(this, name, '')}
          key={Math.random().toString()}
        >
          #{name}
        </span>
        {hasUnreadMessages ? (
          <span className={className} style={this.state.theme}>
        ...
        <span
          ...
          onClick={NetworkActions.leaveChannel.bind(this, name)}
          key={Math.random().toString()}
        >
          Close
        </span>
    ...
    return (
      <div>
        <TransitionGroup
          {...transitionProps}
          transitionName={
            this.state.leftSide ? 'openPanelAnimationRight' : 'openPanelAnimationLeft'
          }
        >
          <div className="ChannelsPanel" style={align}>
            ...
            <TransitionGroup
              {...transitionProps}
              transitionName={
                this.state.leftSide ? 'panelHeaderAnimationRight' : 'panelHeaderAnimationLeft'
              }
            >
            ...
            <TransitionGroup
              ...
              className={channelJoinInputClass}
            >
              <JoinChannel
