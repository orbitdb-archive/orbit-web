    ...
    const peers = this.state.peers.map(p => {
      return (
        <TransitionGroup
          ...
          className="peer"
        >
          <svg width="20" height="8" key={'svg' + p}>
            <circle cx="4" cy="4" r="4" fill="rgba(116, 228, 96, 0.8)" />
          </svg>
          <span key={p}>{p}</span>
        </TransitionGroup>
      )
    ...
    return (
      <div className="SwarmView">
        <div className="summary">
          Your Peer ID is <span className="green">{peerId}</span>
        </div>
        <div className="summary">
          Peers connected <span className="green">{this.state.peers.length}</span>
        </div>
        <div className="peers">{peers}</div>
      </div>
    )
  }
}
