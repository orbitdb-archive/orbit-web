  ...
  onKeyDown (event, inputText, sourceWords) {
    if (event.which === 9) {
      ...
      if (this.tabPressCounter == null) {
        ...
        const lastWord = this.words.pop().toLowerCase()
        this.matches = sourceWords
          .map(f => (lastWord !== '' && f.toLowerCase().startsWith(lastWord) ? f : null))
          .filter(f => f !== null)
      } else {
      ...
      if (this.matches.length > 0) {
        ...
        if (this.onUpdated) this.onUpdated(this.words.join(' '))
      }
    } else {
      this.words = inputText.split(' ')
      this.tabPressCounter = null
