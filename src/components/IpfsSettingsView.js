...
class IpfsSettingsView extends React.Component {
  ...
  onCompoundChange (value, name) {
    const ipfsDaemonSettings = Object.assign({}, this.state.ipfsDaemonSettings)
        ...
        <form className="IpfsSettingsView" onSubmit={this.save.bind(this)}>
          ...
          {!isElectron ? (
            ...
            <div>
              ...
              <div>
                <IpfsApiSettings API={settings.API} onChange={this.onCompoundChange} />
              </div>
            </div>
          )}
        </form>
        <div className="save">
          <button className="submitButton" onClick={this.save.bind(this)}>
            Save
          </button>
        </div>
      </div>
