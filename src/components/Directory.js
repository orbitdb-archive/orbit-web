...
class Directory extends React.Component {
  ...
  onOpen (e) {
    const gateway = isElectron
      ? 'http://localhost:' + window.gatewayAddress.split(':')[1]
      : 'https://ipfs.io./ipfs/'
  ...
  render () {
    ...
    const name = (
      ...
      >
        <a href="" onClick={this.openDirectory.bind(this)}>
          {this.state.name}
        </a>
      </TransitionGroup>
    )
      ...
      children = this.state.children.map(e => {
        return e.type === 'file' ? (
          <File
            ...
            size={e.size}
            key={e.hash}
            onPreviewOpened={this.props.onPreviewOpened}
          />
        ) : (
          <Directory
            hash={e.hash}
    ...
    const showIndexHtml =
      this.state.children.map(e => e.name).includes('index.html') && this.state.open

    return (
      <div className="DirectoryView">
        <div className="Directory">
          {name}
          <span className="download" href={openLink} onClick={this.onOpen.bind(this)}>
            Open
          </span>
        ...
        <div style={{ display: 'flex' }}>
          <div className={children.length > 0 ? 'children' : ''} ref="directory">
            {children}
          </div>
